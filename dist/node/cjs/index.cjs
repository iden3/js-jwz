...
let _iden3_js_iden3_core = require("@iden3/js-iden3-core");
let _iden3_js_crypto = require("@iden3/js-crypto");
let rfc4648 = require("rfc4648");
let snarkjs = require("snarkjs");
let _iden3_js_merkletree = require("@iden3/js-merkletree");
...
function hash(message) {
	const bi = (0, _iden3_js_iden3_core.fromBigEndian)((0, _iden3_js_crypto.sha256)(message).reverse());
	let m = BigInt(0);
	if (checkBigIntInField(bi)) m = bi;
	else m = bi % BigInt(qString);
	return _iden3_js_crypto.poseidon.hash([m]);
}
...
var Token = class Token {
	...
	static async parseCompact(tokenStr) {
		...
		const rawProtected = rfc4648.base64url.parse(parts[0], { loose: true });
		return await new RawJSONWebZeroknowledge(rfc4648.base64url.parse(parts[1], { loose: true }), rawProtected, {}, rfc4648.base64url.parse(parts[2], { loose: true })).sanitized();
	}
	...
	compactSerialize() {
		...
		const serializedProtected = rfc4648.base64url.stringify(this.raw.protectedHeaders, { pad: false });
		const serializedProof = rfc4648.base64url.stringify(this.raw.zkp, { pad: false });
		return `${serializedProtected}.${rfc4648.base64url.stringify(this.raw.payload, { pad: false })}.${serializedProof}`;
	}
	fullSerialize() {
	...
	async getMessageHash() {
		...
		const protectedHeaders = rfc4648.base64url.stringify(serializedHeaders, { pad: false });
		const payload = rfc4648.base64url.stringify(this.raw.payload, { pad: false });
		return (0, _iden3_js_iden3_core.toBigEndian)(hash(encoder.encode(`${protectedHeaders}.${payload}`)), 32);
	}
...
async function verify(messageHash, proof, verificationKey, unmarshall) {
	...
	const expectedChallenge = (0, _iden3_js_iden3_core.fromBigEndian)(messageHash);
...
var provingMethodGroth16AuthV2Instance = new class ProvingMethodGroth16AuthV2 {
	...
	async terminateCurve() {
		(await (0, ffjavascript.getCurveFromName)(ProvingMethodGroth16AuthV2.curveName)).terminate();
	}
	unmarshall(pubSignals) {
		...
		return {
			userID: _iden3_js_iden3_core.Id.fromBigInt(BigInt(pubSignals[0])),
			challenge: BigInt(pubSignals[1]),
			GISTRoot: _iden3_js_merkletree.Hash.fromString(pubSignals[2])
		};
	}
}(new ProvingMethodAlg(Groth16, AuthV2Circuit));
...
var ProvingMethodGroth16AuthV3 = class ProvingMethodGroth16AuthV3 {
	...
	async terminateCurve() {
		(await (0, ffjavascript.getCurveFromName)(ProvingMethodGroth16AuthV3.curveName)).terminate();
	}
	unmarshall(pubSignals) {
		const len = 3;
		if (pubSignals.length !== len) throw new Error(`invalid number of Output values expected ${len} got ${pubSignals.length}`);
